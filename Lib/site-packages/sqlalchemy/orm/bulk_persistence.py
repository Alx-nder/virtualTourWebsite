...
def _bulk_insert(
    ...
    for table, super_mapper in base_mapper._sorted_tables.items():
        ...
        bookkeeping = (
            ...
            or return_defaults
            or (
                use_orm_insert_stmt is not None and bool(use_orm_insert_stmt._returning)
            )
        )
...
class ORMDMLState(AbstractORMCompileState):
    ...
    def _setup_orm_returning(
        ...
        if orm_level_statement._returning:
            fs = FromStatement(orm_level_statement._returning, dml_level_statement)
...
class BulkORMUpdate(BulkUDCompileState, UpdateDMLState):
    ...
    def _setup_for_orm_update(self, statement, compiler, **kw):
        ...
        new_crit = self._adjust_for_extra_criteria(extra_criteria_attributes, mapper)
        if new_crit:
    ...
    def orm_execute_statement(
        ...
        if update_options._dml_strategy not in ("orm", "auto", "bulk"):
            raise sa_exc.ArgumentError(
                "Valid strategies for ORM UPDATE strategy " "are 'orm', 'auto', 'bulk'"
            )
